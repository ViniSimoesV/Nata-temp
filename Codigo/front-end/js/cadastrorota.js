...
function configurarFormulario() {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        if (paradasDaRota.length === 0) {
            alert("Adicione pelo menos um ponto de parada!");
            return;
        }

        // --- LÓGICA DE VERIFICAÇÃO DO TIPO DE ROTA ---
        const temColeta = paradasDaRota.some(p => p.tipo === 'Coleta');
        const temEntrega = paradasDaRota.some(p => p.tipo === 'Entrega');
        
        let tipoCalculado = "Misto"; // Valor padrão
        if (temColeta && !temEntrega) {
            tipoCalculado = "Coleta";
        } else if (!temColeta && temEntrega) {
            tipoCalculado = "Entrega";
        } else if (temColeta && temEntrega) {
            tipoCalculado = "Coleta / Entrega";
        }
        ...
        const novaRota = {
            ...
            status_rota: 1, // 1 para 'Registrada'
            tipo_rota: tipoCalculado,
            carga_total: 0
        };

        try {
            // Salvar a Rota Principal
            const { data: rotaCriada, error: errRota } = await _supabase
                .from('rota')
                .insert([novaRota])
                .select()
                .single();
            ...
            // Salvar as Paradas da tabela: (ponto_parada)
